  ...
  return (
    <div style={{ 
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 clamp(1rem, 4vw, 2rem)', position: 'relative', zIndex: 2 }}>
        <p style={{ fontSize: 'clamp(0.875rem, 1.5vw, 1.15rem)',color: '#009DE0', fontWeight: '800', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '3.5px', textTransform: 'uppercase', marginBottom: '1rem' }}>{label}</p>
        <h1 style={{ fontSize: 'clamp(2.2rem, 8vw, 4rem)', fontWeight: '900', color: '#ffffff', fontFamily: "'Barlow Condensed', sans-serif", textTransform: 'uppercase', lineHeight: '1.1', marginBottom: '1.25rem' }}>{title}</h1>
        {subtitle && <p style={{ fontSize: 'clamp(0.875rem, 1.5vw, 1.15rem)', color: '#EBEBE6', maxWidth: '600px', lineHeight: '1.8' }}>{subtitle}</p>}
      </div>
    </div>
  );
}